...
class Grupo(models.Model):
    ...
    def action_send_email(self):
        ...
        grupo = self.search([('exercicio','=',year),('mes','=',month)])
        for doc in grupo.documento_ids:
            if doc.status_eps == u'unrealized':
                #consultar se o documento dessa requisição já foi assinado
                doc_sei = _SWS.consultar_documento(doc.id_documento)
                if doc_sei.Assinaturas:
                    ...
                    template.write({
                        'email_to': doc.paciente_id.email,
